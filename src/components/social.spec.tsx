...
describe('Social', () => {
  ...
  it('passes correct className to each Block', () => {
    ...
    expect(firstBlock).toHaveClass(
      'rounded-lg border border-zinc-700 p-6 col-span-6 md:col-span-3 relative transition-transform ease-in-out duration-300 hover:scale-105 bg-zinc-50'
    )
    expect(secondBlock).toHaveClass(
      'rounded-lg border border-zinc-700 p-6 col-span-6 md:col-span-3 relative transition-transform ease-in-out duration-300 hover:scale-105 bg-blue-500'
    )
    expect(thirdBlock).toHaveClass(
      'rounded-lg border border-zinc-700 p-6 col-span-6 md:col-span-3 relative transition-transform ease-in-out duration-300 hover:scale-105 bg-black'
    )
    expect(fourthBlock).toHaveClass(
      'rounded-lg border border-zinc-700 p-6 col-span-6 md:col-span-3 relative transition-transform ease-in-out duration-300 hover:scale-105 bg-gradient-to-r from-pink-500 to-yellow-500'
    )
  })
